...
//   const { history, careerPath, caluclateCareer }: { history: QuizItem[], careerPath: string, caluclateCareer: boolean } = body;
...
//     const scorePrompt = `Someone is trying to find their career and what they want to do here are some questions asked so far:
// ${history.map(item => `Q: ${item.question}\nA: ${item.answer}`).join('\n')}

// Based on these responses find the best possible career path from this list: ${jobs}. Provide a brief explanation for the this reason. Return your response in the following JSON format:
// {
//   "career": <string>,
//   "explanation": "<brief explanation>"
// }`;
...
//           {
//             role: "system",
//             content: "You are a career advisor specializing in finding the right role for someone. Evaluate the candidate's responses and provide a possible career score along with a brief explanation."
//           },
//           {
//             role: "user",
//             content: scorePrompt,
